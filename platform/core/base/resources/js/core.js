...
class Guestcms {
    ...
    constructor() {
        ...
        Guestcms.initNavbarMinimal()
        Guestcms.initResources()
        Guestcms.initGlobalResources()
        Guestcms.handleCounterUp()
        Guestcms.initMediaIntegrate()

        if (
            GuestcmsVariables &&
            GuestcmsVariables.authorized === '0' &&
            typeof GuestcmsVariables.authorize_url !== 'undefined'
        ) {
    ...
    static blockUI(options) {
        ...
        Guestcms.showLoading(options.target)
    }

    static unblockUI(target) {
        Guestcms.hideLoading(target)
    }
    ...
    static showNotice(messageType, message, messageHeader = '') {
        ...
        if (Guestcms.noticesTimeout[key]) {
            clearTimeout(Guestcms.noticesTimeout[key])
        }

        Guestcms.noticesTimeout[key] = setTimeout(() => {
            if (!messageHeader) {
                switch (messageType) {
                    case 'error':
                        messageHeader = GuestcmsVariables.languages.notices_msg.error
                        break
                    case 'success':
                        messageHeader = GuestcmsVariables.languages.notices_msg.success
                        break
                }
            }
    ...
    static handleError(data) {
        if (typeof data.errors !== 'undefined' && !_.isArray(data.errors)) {
            Guestcms.handleValidationError(data.errors)
        } else {
            if (typeof data.responseJSON !== 'undefined') {
                if (typeof data.responseJSON.errors !== 'undefined') {
                    if (data.status === 422) {
                        Guestcms.handleValidationError(data.responseJSON.errors)
                    }
                } else if (typeof data.responseJSON.message !== 'undefined') {
                    Guestcms.showError(data.responseJSON.message)
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            Guestcms.showError(item)
                        })
                    })
                }
            } else {
                Guestcms.showError(data.statusText)
            }
        }
    }

    static handleDatatableError(error) {
        let errorMessage = GuestcmsVariables.languages.tables.error_loading
            ? GuestcmsVariables.languages.tables.error_loading
        ...
        Guestcms.showError(errorMessage);
    }
    ...
    static async copyToClipboard(textToCopy, parentTarget) {
        if (navigator.clipboard && window.isSecureContext) {
            ...
            Guestcms.unsecuredCopyToClipboard(textToCopy, parentTarget)
        }
    }
    ...
    static initResources() {
        ...
        $.each($(document).find('.select-search-ajax'), function (index, element) {
            ...
            if ($element.data('url')) {
                ...
                Guestcms.select(element, options)
        ...
        Guestcms.initDatePicker('.datepicker')

        if (jQuery().textareaAutoSize) {
            $('textarea.textarea-auto-height').textareaAutoSize()
        }

        Guestcms.initCodeEditorComponent()
        Guestcms.initColorPicker()
        Guestcms.initLightbox()
        Guestcms.initTreeCategoriesSelect()
        Guestcms.initCoreIcon()
    ...
    static initGlobalResources() {
        ...
        Guestcms.initFieldCollapse()
        Guestcms.initTreeCheckboxes()
        Guestcms.initClipboard()
        Guestcms.initDropdownCheckboxes()
    }
    ...
    handleTurnOffDebugMode() {
        ...
        $submit.on('click', function (event) {
            ...
            Guestcms.showButtonLoading($submit[0])
            ...
            $httpClient
                ...
                .finally(() => {
                    Guestcms.hideButtonLoading($submit[0])
                })
        })
    }
    ...
    static initMediaIntegrate() {
        if (jQuery().rvMedia) {
            Guestcms.gallerySelectImageTemplate = `
            ...
            const gallerySelectImages = function (files, $currentBoxList, excludeIndexes = []) {
                let template = Guestcms.gallerySelectImageTemplate
    ...
    static initCodeEditor(id, type = 'css') {
        ...
        $('.handle-tool-drag').mousedown((event) => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Guestcms.onDragTool)
            $(window).on('mouseup', Guestcms.onReleaseTool)
        })
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', Guestcms.onDragTool)
        $(window).off('mouseup', Guestcms.onReleaseTool)
    }

    processAuthorize() {
        $httpClient
            .makeWithoutErrorHandler()
            .post(GuestcmsVariables.authorize_url)
            .catch(() => {})
    }
    ...
    countMenuItemNotifications() {
        ...
        if ($menuItems.length) {
            $httpClient
                .make()
                .get($menuItems.data('url') || GuestcmsVariables.menu_item_count_url)
                .then(({ data }) => {
                    data.data.map((x) => {
                        if (x.value > 0) {
        ...
        $(document).on('click change', '[data-bb-toggle="collapse"]', (e) => {
            ...
            switch (type) {
                ...
                case 'button':
                    ...
                    break
                default:
                    console.warn(`[Guestcms] Unknown type ${type} of collapse`)
            }
        })
    ...
    static initCodeEditorComponent() {
        $(document)
            ...
            .each(function () {
                ...
                Guestcms.initCodeEditor(this, this.dataset.mode || 'htmlmixed')
            })
    }
    ...
    static initLightbox() {
        ...
        a.forEach((element) => {
            ...
            if (!instance[instanceName]) {
                instance[instanceName] = Guestcms.lightbox()
            }
    ...
    static initClipboard() {
        $(document).on('click', '[data-bb-toggle="clipboard"]', async (e) => {
            ...
            await Guestcms.copyToClipboard(text, clipboardParentTarget)

            if (copiedMessage) {
                Guestcms.showSuccess(copiedMessage)
            }
        ...
        $element.editable({
            ...
            success: function (response) {
                if (response.error && response.message) {
                    Guestcms.showError(response.message)
                }
            },
            error: function (response) {
                Guestcms.handleError(response)
            },
        })
    }
