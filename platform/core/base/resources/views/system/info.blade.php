...
@section('content')
    @php $serverIp = Guestcms\Base\Supports\Helper::getIpFromThirdParty(); @endphp
    <x-core::alert type="primary" :title="trans('core/base::system.report_description')">
        <x-core::button type="button" id="btn-report" color="info" size="sm" class="mt-2">
            {{ trans('core/base::system.get_system_report') }}
        </x-core::button>

        <div class="mt-3" id="report-wrapper" style="display: none;">
            <textarea name="txt-report" id="txt-report" class="form-control" rows="10" spellcheck="false"
                onfocus="this.select()">
            ...
            <x-core::button type="button" id="copy-report" size="sm" class="mt-2" data-bb-toggle="clipboard"
                data-clipboard-action="copy" data-clipboard-message="Copied" data-clipboard-target="#txt-report">
        ...
        <div class="col-sm-4">
            <x-core::card class="mb-3" data-get-addition-data-url="{{ route('system.info.get-addition-data') }}">
                ...
                <ul class="list-group list-group-flush">
                    ...
                    <li class="list-group-item">
                        {{ trans('core/base::system.server_ip') }}: <span class="me-1">{{ $serverIp }}</span> <x-core::copy
                            :copyableState="$serverIp" />
                    </li>
                    <li class="list-group-item">
                        {{ trans('core/base::system.debug_mode_off') }}:
                        @include('core/base::system.partials.status-icon', ['status' => !$systemEnv['debug_mode']])
                    </li>
                    <li class="list-group-item">
                        {{ trans('core/base::system.storage_dir_writable') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $systemEnv['storage_dir_writable']])
                    <li class="list-group-item">
                        {{ trans('core/base::system.cache_dir_writable') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $systemEnv['cache_dir_writable']])
                    </li>
                    <li class="list-group-item">
                        {{ trans('core/base::system.app_size') }}: <span id="system-app-size"><span
                                class="spinner-border spinner-border-sm text-secondary" role="status"></span></span>
                    </li>
                </ul>
            </x-core::card>
            ...
            <x-core::card>
                ...
                <ul class="list-group list-group-flush">
                    ...
                    <li class="list-group-item">
                        ...
                        {!! $serverEnv['max_execution_time'] ?: '&mdash;' !!}
                    </li>
                    <li class="list-group-item">
                    ...
                    <li class="list-group-item">
                        {{ trans('core/base::system.ssl_installed') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $serverEnv['ssl_installed']])
                    </li>
                    <li class="list-group-item">
                    ...
                    <li class="list-group-item">
                        {{ trans('core/base::system.allow_url_fopen_enabled') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $serverEnv['allow_url_fopen_enabled']])
                    </li>
                    <li class="list-group-item">
                        {{ trans('core/base::system.openssl_ext') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $serverEnv['openssl']])
                    </li>
                    <li class="list-group-item">
                        {{ trans('core/base::system.mbstring_ext') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $serverEnv['mbstring']])
                    </li>
                    <li class="list-group-item">
                        {{ trans('core/base::system.pdo_ext') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $serverEnv['pdo']])
                    </li>
                    <li class="list-group-item">
                        {{ trans('core/base::system.curl_ext') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $serverEnv['curl']])
                    <li class="list-group-item">
                        {{ trans('core/base::system.exif_ext') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $serverEnv['exif']])
                    <li class="list-group-item">
                        {{ trans('core/base::system.file_info_ext') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $serverEnv['fileinfo']])
                    </li>
                    <li class="list-group-item">
                        {{ trans('core/base::system.tokenizer_ext') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $serverEnv['tokenizer']])
                    </li>
                    <li class="list-group-item">
                        {{ trans('core/base::system.imagick_or_gd_ext') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $serverEnv['imagick_or_gd']])
                    </li>
                    <li class="list-group-item">
                        {{ trans('core/base::system.zip') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $serverEnv['zip']])
                    <li class="list-group-item">
                        {{ trans('core/base::system.iconv') }}:
                        @include('core/base::system.partials.status-icon', ['status' => $serverEnv['iconv']])
                    </li>
                </ul>
            </x-core::card>
        </div>
