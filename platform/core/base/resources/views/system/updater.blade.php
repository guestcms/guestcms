@php
    use Guestcms\Base\Enums\SystemUpdaterStepEnum;
...
@section('content')
    <div style="max-width: 900px;" class="m-auto">
        ...
        @if (($memoryLimit && $memoryLimit != -1 && $memoryLimit < $requiredMemoryLimit) || ($maximumExecutionTime && $maximumExecutionTime < $requiredMaximumExecutionTime))
            <x-core::alert type="warning" title="Warning" :important="false">
                <div class="mt-3 mb-0">
                    ...
                    <p class="mt-3 mb-0">
                        ...
                        <span>Check out the <a
                                ...
                                rel="noreferer" target="_blank">PHP Memory Limit Configuration Guide (CPanel)</a> for
        ...
        @if (!$activated)
            <x-core::alert type="warning" title="You haven't activated your license yet!" :important="true">
                <p class="mt-3 mb-0">
            ...
            <x-core::card class="mb-3">
                ...
                <x-core::card.body dir="ltr">
                    @if (!empty($latestUpdate))
                        <system-update-component update-url="{{ route('system.updater.post') }}"
                            update-id="{{ $latestUpdate->updateId }}" version="{{ $latestUpdate->version }}"
                            first-step="{{ SystemUpdaterStepEnum::firstStep() }}"
                            first-step-message="{{ SystemUpdaterStepEnum::DOWNLOAD()->message() }}"
                            last-step="{{ SystemUpdaterStepEnum::lastStep() }}" :is-outdated="{{ Js::encode($isOutdated) }}"
                            :is-activated="{{ Js::encode($activated) }}" v-slot="{ performUpdate }">
                            @if (!$isOutdated)
                                <h3 class="text-success">{{ __('The system is up-to-date. There are no new versions to update!') }}</h3>
                            @endif

                            @if (!$activated)
                                <x-core::modal.action id="system-updater-confirm-modal" type="warning" title="Are you sure?"
                                    description="Your license has not been activated yet! You might not receive the latest updates."
                                    submit-button-label="Yes, update it!" :submit-button-attrs="['@click' => 'performUpdate']"
            ...
            <x-core::card class="mb-3">
                ...
                <x-core::card.body dir="ltr">
                    <x-core::alert type="info" class="mb-3">
                        Having trouble with the One-Click System Updater? No worries! Check out the manual updater below - it's
                        ...
                        <x-core::step :vertical="true" :counter="true">
                            @foreach (SystemUpdaterStepEnum::labels() as $step => $label)
                                ...
                                <x-core::step.item>
                                    ...
                                    <x-core::button class="mb-3" type="submit" name="step_name" value="{{ $step }}" color="primary"
