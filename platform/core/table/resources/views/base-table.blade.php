                        ...
                        @if (is_string($defaultButton))
                            @switch($defaultButton)
                                ...
                                @case('export')
                                    ...
                                    @break
                                @case('visibility')
                                    @break(! Auth::user() instanceof \Guestcms\ACL\Models\User || ! AdminHelper::isInAdmin(true))
                                        ...
                                        <div class="dropdown-menu p-2">
                                            <x-core::form :url="route('table.update-columns-visibility')" method="PUT" data-bb-toggle="dt-columns-visibility">
                                                ...
                                                @foreach($table->getColumns() as $column)
                                                    @php /** @var \Guestcms\Table\Columns\Column $column */ @endphp

                                                    @continue(! $column instanceof \Guestcms\Table\Columns\Column || Str::contains($column->className, 'no-column-visibility') || in_array($column->name, $table->getDefaultVisibleColumns(), true))
