...
<head>
    ...
    <meta name="description" content="Installation Guestcms CMS v{{ get_cms_version() }}">
            ...
            <script type="text/javascript">
                $(function () {
                    @if (Session::has('success_msg'))
                        Guestcms.showSuccess('{{ session('success_msg') }}');
                    @endif
                    @if (Session::has('error_msg'))
                        Guestcms.showError('{{ session('error_msg') }}');
                    @endif
                    @if (isset($error_msg))
                        Guestcms.showError('{{ $error_msg }}');
                    @endif
                    @if (isset($errors))
                        @foreach ($errors->all() as $error)
                            Guestcms.showError('{{ $error }}');
                        @endforeach
                    @endif
                        })
            </script>
