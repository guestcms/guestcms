<x-core::card class="report-chart-content" id="report-chart">
    <x-core::card.header>
        <h4 class="card-title">{{ trans('plugins/hotel::booking-reports.booking_reports') }}</h4>
    </x-core::card.header>
    <x-core::card.body>
        <div class="row">
                ...
                @if (collect($count['revenues'])->isNotEmpty())
                    <div class="rp-card-chart position-relative mb-3">
                        ...
                        <div class="rp-card-information">
                            ...
                            @endforeach
                            <small>{{ trans('plugins/hotel::booking-reports.total_earnings') }}</small>
                        </div>
                    </div>
...
@if (request()->ajax())
    @include('plugins/hotel::reports.widgets.chart-script')
@else
    @push('footer')
        @include('plugins/hotel::reports.widgets.chart-script')
    @endpush
