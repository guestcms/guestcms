...
<li class="list-group-item payment-method-item">
    ...
    <div @class(['payment_collapse_wrap collapse mt-1', 'show' => $isSelected])>
        ...
        @php
            ...
            $feeType = get_payment_setting('fee_type', $name, \Guestcms\Payment\Enums\PaymentFeeTypeEnum::FIXED);
            $orderAmount = apply_filters('payment_order_total_amount', 0);
            $fee = \Guestcms\Payment\Supports\PaymentFeeHelper::calculateFee($name, $orderAmount);
        @endphp
        @if ($feeValue > 0)
            <p class="text-warning">
                @if ($feeType === \Guestcms\Payment\Enums\PaymentFeeTypeEnum::PERCENTAGE)
