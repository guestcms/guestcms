@if (get_payment_setting('status', RAZORPAY_PAYMENT_METHOD_NAME) == 1)
    @php
        $paymentService = new Guestcms\Razorpay\Services\Gateways\RazorpayPaymentService();
    @endphp

    <x-plugins-payment::payment-method :name="RAZORPAY_PAYMENT_METHOD_NAME" paymentName="Razorpay"
    ...
    @if (
            $paymentService->isValidToProcessCheckout() && get_payment_setting(
                'payment_type',
                        ...
                        Stripe.card.createToken(form, function (status, response) {
                            if (response.error) {
                                if (typeof Guestcms != 'undefined') {
                                    Guestcms.showError(response.error.message, _self.data(
                                        'error-header'));
                                } else {
